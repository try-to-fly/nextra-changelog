...
export const ChangeLog = () => {
  ...
  const [pageDatas, setPageDatas] = useState<Record<string, { title: string }>>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const pageDatas = await ky
        .get('https://wiki.dev-hub.top/_next/static/chunks/nextra-data-en-US.json')
        .json<any>();
    ...
    <div>
      <div className={styles.header}>
        ...
        <div className={styles.type}>
          <span className={type === 'week' ? styles.active : ''} onClick={() => setType('week')}>
            本周
          </span>
          <span className={type === 'month' ? styles.active : ''} onClick={() => setType('month')}>
            本月
          </span>
        </div>
      </div>
